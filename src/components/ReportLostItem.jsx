...
const ReportLostItem = ({ onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await itemsAPI.reportLost(formDataToSend);
      onSuccess?.();
    } catch (err) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6 max-w-2xl mx-auto max-h-[90vh] overflow-y-auto">
      <div className="flex items-center gap-3 mb-6">
        <div className="bg-gradient-to-br from-rose-100 to-rose-200 rounded-lg p-2">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="w-6 h-6 text-rose-600">
        ...
        <div>
          <h3 className="text-xl font-bold text-[#2f303c]">Report Lost Item</h3>
          <p className="text-sm text-[#5C5B61]">Help us find your lost item</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-semibold text-[#2f303c] mb-2">
            Photo (Optional - AI will auto-detect details)
          </label>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-[#5C5B61] mt-1">AI will extract details from photo</p>
            </div>
          </div>
        </div>
        ...
        {/* Location */}
        <div>
          <label className="block text-sm font-semibold text-[#2f303c] mb-2">Where did you lose it?</label>
          <input
            type="text"
        ...
        {/* Lost Date */}
        <div>
          <label className="block text-sm font-semibold text-[#2f303c] mb-2">When did you lose it?</label>
          <input
            type="date"
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2 rounded-lg border-2 border-gray-200 focus:outline-none focus:border-[#4278ff] focus:ring-4 focus:ring-[#4278ff]/10"
            placeholder="e.g., What's the serial number?"
          />
          <p className="text-xs text-[#5C5B61] mt-1">Ask a question only the real owner can answer</p>
        </div>

        {/* Unique Traits */}
        <div>
          <label className="block text-sm font-semibold text-[#2f303c] mb-2">Unique Traits</label>
          <textarea
            value={formData.unique_traits}
            onChange={(e) => setFormData({ ...formData, unique_traits: e.target.value })}
            rows={2}
            className="w-full px-4 py-2 rounded-lg border-2 border-gray-200 focus:outline-none focus:border-[#4278ff] focus:ring-4 focus:ring-[#4278ff]/10"
            placeholder="Scratches, stickers, serial numbers, etc."
          />
        </div>
